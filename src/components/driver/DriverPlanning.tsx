      ...
      <div className="p-4 rounded-2xl bg-driver-card border border-driver-border shadow-2xl">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="relative group">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-blue-500 transition-colors" size={18} weight="bold" />
            <input
              type="text"
              placeholder="Nom du client, téléphone..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm focus:ring-2 focus:ring-blue-500 transition-all text-white outline-none"
            />
          </div>
          ...
          <div className="flex gap-4">
            <div className="relative flex-1">
              <Filter className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={16} weight="bold" />
              <select
                value={statusFilter}
            ...
            <div className="relative flex-1">
              <CalendarIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={16} weight="bold" />
              <select
                value={dateFilter}
          ...
          <div className="flex items-center justify-end">
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-xs font-bold uppercase text-gray-400 hover:text-white transition-all"
            >
              Actualiser
            </button>
          </div>
              ...
              >
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:bg-blue-500/10 transition-colors">
                      <span className="text-xl font-bold text-white group-hover:text-blue-400">
                        {booking.client.charAt(0)}
                      </span>
                    </div>
                    <div>
                      <h3 className="font-bold text-white tracking-tight">{booking.client}</h3>
                ...
                <div className="grid grid-cols-1 gap-3 py-4 border-y border-white/5">
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-xs text-gray-300 line-clamp-1">{booking.pickup}</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <Navigation className="text-blue-500 mt-1 shrink-0" size={14} weight="fill" />
                    <span className="text-xs text-gray-300 line-clamp-1">{booking.destination}</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center mt-4">
                  ...
                  <div className="text-right">
                    <span className="text-xs font-mono text-emerald-500 font-bold">{booking.price.toLocaleString()} FCFA</span>
                  </div>
                </div>
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 space-y-4">
                    <div>
                      ...
                      <p className="text-sm font-medium text-white leading-tight">{selectedBooking.pickup}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold uppercase text-gray-500 mb-1 tracking-widest">Destination</p>
                      <p className="text-sm font-medium text-white leading-tight">{selectedBooking.destination}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                  ...
                  <div className="flex items-center gap-2 text-white font-mono">
                    <Clock size={16} weight="bold" className="text-blue-400" /> {selectedBooking.time}
                  </div>
                </div>
                <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                ...
                <div className="col-span-2 p-4 rounded-2xl bg-white/5 border border-white/10">
                  ...
                  <div className="text-white font-bold flex items-center gap-2">
                    <Car size={16} weight="light" className="text-gray-400" /> {selectedBooking.vehicle}
                  </div>
                </div>
              </div>
              ...
              {selectedBooking.notes && (
                <div className="p-4 rounded-xl bg-orange-500/5 border border-orange-500/20">
                  <p className="text-[10px] font-bold uppercase text-orange-400 mb-1">Instructions Supplémentaires</p>
                  <p className="text-xs text-orange-100/70">{selectedBooking.notes}</p>
                </div>
              )}
                ...
                {(selectedBooking.status === 'assigned' || selectedBooking.status === 'pending') ? (
                  <div className="grid grid-cols-2 gap-4">
                    <button
                      ...
                      disabled={isUpdating}
                      className="flex items-center justify-center gap-2 py-4 rounded-2xl bg-white/5 border border-white/10 text-white font-bold hover:bg-red-500/20 hover:border-red-500/40 transition-all disabled:opacity-50"
                    >
